...
def build_disk_map(raw_data: List[int]):
    """
...
def create_file_catalog(disk: list[int]) -> dict:
    file_catalog = {}

    for i, val in enumerate(disk):
        if val != -1:
            if val in file_catalog.keys():
                file_catalog[val]['size'] += 1
            else:
                file_catalog[val] = {'start': i, 'size': 1}
...
def main():
    ...
    file_catalog = create_file_catalog(initial_disk)
    # print(initial_disk)
    for file_id in sorted(file_catalog.keys(), reverse=True):
        gap_space = 0
        for i, val in enumerate(initial_disk):
            if i > file_catalog[file_id]['start']:
                break
            elif val == -1:
                gap_space += 1
            else:
            ...
            if gap_space == file_catalog[file_id]['size']:
                start = file_catalog[file_id]['start']
                stop = file_catalog[file_id]['start'] + file_catalog[file_id]['size']
                for loc in range(start, stop):
                    initial_disk[loc] = -1

                start = (i - file_catalog[file_id]['size']) + 1
                stop = i + 1
                for x in range(start, stop):
                    initial_disk[x] = file_id
    ...
    checksum = calculate_checksum(initial_disk)
